...
var viewedQueue chan *Viewed

func init() {
	viewedQueue = make(chan *Viewed, 64)
	go viewedWorker()
}

func viewedWorker() {
	for vv := range viewedQueue {
		doSetViewed(vv)
	}
}

func doSetViewed(vv *Viewed) {
...
func SetViewed(vv *Viewed) {
	...
	case viewedQueue <- vv:
	default:
		log.TLogln("Viewed queue full, dropping:", vv.Hash, vv.FileIndex)
	}
}
